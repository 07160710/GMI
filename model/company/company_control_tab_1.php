...
<?php include("../../public/public_project_js.php"); ?>
<?php include("../../public/public_body_js.php"); ?>
            ...
            $.ajax({
                ...
                success: function (xml) {
                    ...
                    $(xml).find('company').each(function(){
                        ...
                        ?>
                        //var status = (cancelled=='是')?'cancelled':'';

                        var alt_class = '';
                        if(i%2!=0)alt_class = 'alt';

                        $('#company_list').append(
                            '<tr id="company_'+company_id+'" class="data_row '+alt_class+' '+status+'">' +
                            '<td>' +
                            '<input type="checkbox">' +
                            '</td>' +
                            '<td align="center">' +
                            ...
                            foreach($GLOBALS['company_list_fields'] as $key=>$val){
                                ...
                                if($key=="code")print "'<td $align_str><input type=\"hidden\" id=\"code_'+company_id+'\" value=\"'+$key+'\"><a class=\"copy_btn\" id=\"copy_code_'+company_id+'\" title=\"复制到粘贴板\">'+$key+'</a></td>' + ";
                                else print "'<td $align_str>'+$key+'</td>' + ";
                            }
                            ?>
                            '</tr>'
                        );
