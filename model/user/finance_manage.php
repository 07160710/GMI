...
if($_POST['action']=="assign_finance"){
	if(isset($_REQUEST['scope']) && $_REQUEST['scope']=="all"){
		$cond_str = build_responsible_cond();
		include_once("fetch_project_cond.php");
		$sql = "SELECT project.id 
		...
		if(mysql_num_rows($get_project)>0){
			while($row = mysql_fetch_array($get_project)){
				...
				if($project_info['year_apply']>=date('Y')){
					$sql = "UPDATE project_assign SET is_curr=0 WHERE project_id='$id' AND u_type='f'";
					if(!mysql_query($sql)){
						$arr = array(
							'success'=>0,
							'error'=>"设置财务派单过往记录出错：".mysql_error()
						);
						echo json_encode($arr);
						exit;
					}
					...
					if(isset($_POST['curr_assign'])){
						foreach($_POST['curr_assign'] as $u_id){
							...
							$sql = "SELECT COUNT(*) FROM project_assign WHERE project_id='$id' AND u_type='f' AND u_id='$u_id'";
							...
							if($has_rec==0){
								...
								if(mysql_query($sql)){
									wxmsg_assign_task("f", $id, $u_id, $start_date);
									save_log("f",$id,"派单");
								}
								else{
									$arr = array(
										'success'=>0,
										'error'=>"保存财务派单当前记录出错：".mysql_error()
									);
									echo json_encode($arr);
									exit;
								}
								...
								$sql = "UPDATE project_assign 
										...
										WHERE project_id='$id' AND u_type='f' AND u_id='$u_id'";
								if(!mysql_query($sql)){
									$arr = array(
										'success'=>0,
										'error'=>"保存财务派单当前记录出错：".mysql_error()
					...
					if(isset($_POST['pass_assign'])){
						foreach($_POST['pass_assign'] as $u_id){
							...
							if(!mysql_query($sql)){
								$arr = array(
									'success'=>0,
									'error'=>"保存财务派单过往记录出错：".mysql_error()
								);
								echo json_encode($arr);
								exit;
							}
...
if($_POST['action']=="edit"){
	...
	$db_assign_arr = [];
	$sql = "SELECT u_id FROM project_assign WHERE project_id='$id' AND u_type='f'";
	...
	foreach($db_assign_arr as $db_assign){
		if(!in_array($db_assign,$assign_arr)){
			$sql = "DELETE FROM project_assign WHERE project_id='$id' AND u_id='$db_assign' AND u_type='f'";
			if(!mysql_query($sql)){
				$arr = array(
					'success'=>0,
	...
	$update_field_str = "";
	foreach($GLOBALS['finance_fields'] as $key){
		if($key!="id"){			
...
if($_REQUEST['action']=="export_project"){
	...
	$select_key_str = "";
	foreach($GLOBALS['finance_list_fields'] as $key=>$val){
		if(	$key!="sales" && 
			$key!="technology" && 
			$key!="finance"
		){
